 ...
 *     if (Loader.isModLoaded("mira")) {
 *         return getSizeImpl(entity);
 *     }
 *     return 1.0;
 * }
 *
 * {@literal @}Optional.Method(modid = "mira")
...
public interface IRenderSized extends ISized {
     ...
     * <p>
     * However, other appropriate actions are done, such as dismounting,
     * getting up from the bed, and calling this method for
     * {@link Entity#getParts() entity subparts} recursively.
     * <p>
     * The size is set immediately if the <code>animationTicks</code> is zero,
     * otherwise the process of smooth size change is started, lasting for
     * <code>animationTicks</code> ticks.
     * <p>
     * The formula for default recalibrator lerp time is:
     * <p>
     * <code>int animationTicks = (int) (abs(log(fromSize) - log(toSize)) * TWO_OVER_LOG_TWO)</code>
     *
     * @param size           the size to be set
     * @param animationTicks the time required for the size to be changed from
     *                       current to required
     */
    void setSizeCM(double size, int animationTicks);
     ...
     */
    @Nullable
    IResizingProgress getResizingCM();

    /**
     * <p>
     * Directly sets the size for this entity instance, similar to the original
     * {@link ISized#setSizeCM}
     * contract.
     * </p>
     * <p>
     * No network sync, no lerping and no extra resize checks/actions (such as
     * dismounting).
     * </p>
     * <p>
     * Sets the field (to be returned by {@link #getSizeCM()}) and updates the
     * hitbox, thats it.
     * </p>
     * <p>
     * Used by other entity size setters, exposed in public API to allow precise
     * size control
     * with no overhead from the default method.
     * </p>
     *
